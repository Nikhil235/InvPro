...
export function SignalCard() {
  ...
  return (
    <Card className={`col-span-12 lg:col-span-4 bg-gradient-to-br from-card ${isLong ? 'to-bullish-muted/10 border-bullish/20' : isShort ? 'to-bearish-muted/10 border-bearish/20' : 'to-neutral-muted/10 border-neutral/20'}`}>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className={`text-4xl font-bold ${themeColor}`}>{direction}</span>
            <Badge variant={badgeVariant}>LIVE</Badge>
          </div>
          <p className="text-secondary text-sm">Updated at {formatTime(data?.timestamp)}</p>
        </div>
